...
let deletedNotesRepo = {
    ...
    postDeletedNotes: function(){
        // this.main.innerHTML += `<div ><button style='padding:20px;background-color:white;'>clear All</button></div>`
        let mainTemp = `<div id='maint' class='row'></div>`
        this.main.innerHTML += mainTemp;
        let temp = document.getElementById('maint');
        console.log(this.deletedNotesArr);
        for(let i = 0;i < this.deletedNotesArr.length;i++){
            this.html = Mustache.to_html(yellowNoteExampleTemp(), this.deletedNotesArr[i]);
...
let createNote = {
    ...
    getcache: function(){
        ...
        this.timeRegex = /(\d{1,2}:\d{1,2}:Pm|pm|pM)|(\d{1,2}:\d{1,2}:Am|am|aM)/g
...
let startApp = {
    ...
    postIncomeNotes: function(){
        ...
        for(let i = 0; i < notesRepo.noteArr.length;i++){
        this.html = Mustache.to_html(yellowNoteExampleTemp(),notesRepo.noteArr[i]);
    ...
    timeHelper: function(){
        ...
        if(this.timeLength === 6 && this.date_time.value.match(this.amReg)) return this.date_time.value = this.date_time.value.replace (this.amReg , 'Am');
        if(this.timeLength === 6 && this.date_time.value.match(this.pmReg)) return this.date_time.value = this.date_time.value.replace (this.pmReg ,'Pm');
        if(this.timeLength === 1 && this.date_time.value.match(this.hourReg) || this.timeLength === 2 && this.date_time.value.msearch(this.hourReg))  this.date_time.value += ':';
        if(this.date_time.value.match(this.minutesReg))  this.date_time.value += ':';
    },
    render:function(){
        this.renderNotes();
        this.cacheDom();
    ...
    renderNotes:function(){
        ...
        for(let k = 0; k < notesRepo.noteArr.length;k++){
            this.notesPlace.innerHTML += Mustache.to_html(yellowNoteExampleTemp(),notesRepo.noteArr[k]);
        }
        },500);
    },
    ...
    //--------END Of date--------//
    //--------Start of time--------//
    this.hourReg = /([2-9]{1})|(1\d)|(0\d{1})/g;
    this.minutesReg = /:(\d{2})/g;
    this.amReg = /[aA]/g;
    this.pmReg = /[pP]/g;
    //--------END Of time--------//
    ...
    checkTasks: function(){
        ...
        for(let i =0;i < notesRepo.noteArr.length;i++){
            ...
            if(notesRepo.noteArr[i].date.match(dayReg) < date.getDate() && notesRepo.noteArr[i].date.match(monthReg) < date.getMonth()) return ask() + alert(date.getDate());
        }
    }
}
